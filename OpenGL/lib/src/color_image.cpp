...
std::vector<float> ColorImage::calc_d() {
  if (GLCompute::isAvailable()) {
    if (mQuantize) {
      return GLCompute::calcDQ(mData.data(), mN, mW, mH, mQdata, mTheta,
                               mAlpha);
    } else {
      return GLCompute::calcDNQ(mData.data(), mN, mW, mH, mTheta, mAlpha);
    }
  }
...
std::vector<float> ColorImage::r_calc_d(int r) {
  if (GLCompute::isAvailable()) {
    return GLCompute::rCalcD(mData.data(), mW, mH, r, mTheta, mAlpha);
  }
