...
class ArgParser {
 public:
  ArgParser(std::string const& helptext = "", std::string const& version = "")
  ...
  ArgParser& command(std::string const& name, std::string const& helptext = "",
                     void (*callback)(std::string cmd_name,
                                      ArgParser& cmd_parser) = nullptr);
 ...
 private:
  ...
  void parseEqualsOption(std::string prefix, std::string name,
                         std::string value);
  void exitHelp();
  void exitVersion();
};
