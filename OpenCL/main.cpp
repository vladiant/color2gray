...
--r       Mu
--q       Q
--gpu     Use OpenCL GPU acceleration
)doc";
...
int main(int argc, char** argv) {
  ...
  const bool use_gpu = parser.found("gpu");
...
#ifdef HAS_OPENCL
  // Initialize OpenCL if --gpu was requested
  std::cout << "OpenCL initialization...\n";
  std::unique_ptr<OpenCLContext> ocl;
  if (use_gpu) {
    ocl = std::make_unique<OpenCLContext>();
    if (!ocl->isValid()) {
      std::cerr << "OpenCL initialization failed, falling back to CPU.\n";
      ocl.reset();
    }
  }
#endif
...
#ifdef HAS_OPENCL
  if (ocl) {
    const TimeBench bench{"load (OCL)"};
    initial_image.load_ocl(sourceData, sourceWidth, sourceHeight, *ocl);
  } else
#endif
  {
...
#ifdef HAS_OPENCL
      if (ocl)
        d = initial_image.r_calc_d_ocl(r, *ocl);
      else
#endif
...
#ifdef HAS_OPENCL
      if (ocl)
        d = initial_image.calc_d_ocl(*ocl);
      else
#endif
