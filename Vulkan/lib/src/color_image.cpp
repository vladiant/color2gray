...
std::vector<float> ColorImage::calc_d() {
  ...
  // Try Vulkan GPU path
  if (mVulkan && mVulkan->isAvailable()) {
    // Pack amy_lab data into vec4-aligned floats: [l, a, b, 0]
    std::vector<float> labPacked(mN * 4);
    for (int i = 0; i < mN; i++) {
      labPacked[i * 4 + 0] = mData[i].l;
      labPacked[i * 4 + 1] = mData[i].a;
      labPacked[i * 4 + 2] = mData[i].b;
      labPacked[i * 4 + 3] = 0.0f;
    }

    if (mQuantize) {
      // Pack quantized data: [l, a, b, count]
      std::vector<float> qPacked(mQdata.size() * 4);
      for (size_t i = 0; i < mQdata.size(); i++) {
        qPacked[i * 4 + 0] = mQdata[i].first.l;
        qPacked[i * 4 + 1] = mQdata[i].first.a;
        qPacked[i * 4 + 2] = mQdata[i].first.b;
        qPacked[i * 4 + 3] = static_cast<float>(mQdata[i].second);
      }
      mVulkan->calcDQ(mN, labPacked.data(),
                      static_cast<int>(mQdata.size()), qPacked.data(),
                      mTheta, mAlpha, d.data());
    } else {
      mVulkan->calcDNQ(mN, labPacked.data(), mTheta, mAlpha, d.data());
    }
    return d;
  }
